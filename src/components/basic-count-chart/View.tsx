...
const View: React.FC<ViewProps> = ({
  ...
  const id = random ? random : 'basic-count-chart';
  const { chartData, fetchError, isNoData } = useChartCountQuery(
    tenant,
    workspace,
    sessionToken,
    option,
  ...
  const view = (
    <div id={id} className={cx(isListView ? 'basic-chart-count-list' : 'basic-chart-count-detail')}>
      <span className={cx('view-filter-data')}>
        <span>{chartData?.payload?.result}</span>
        <span style={{ float: option?.unit }}>{option?.unitName}</span>
      </span>
    </div>
  );
